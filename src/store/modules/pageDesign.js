...
const state = {
  ...
  dPagesList: {}, // 所有页面
  dWidgets: [], // 当前页面已使用的组件
  dHistory: [], // 当前页面记录历史操作（直接保存整个画布的json数据）
...
const getters = {
  dPagesIndex (state) {
    return state.dPagesIndex
  },
  dPagesList (state) {
    return state.dPagesList
  },
  dZoom (state) {
    return state.dZoom
  },
...
const actions = {
  ...
  * 初始化页面
  */
  initPages (store, data) {
    console.log(setting)
    Object.assign(store.state.dPage, data.dPage)
    data.pages.forEach(it => {
      it.dWidgets.forEach((jt, j) => {
        let set = JSON.parse(JSON.stringify(setting[jt.type]))
        set.uuid = generate('1234567890abcdef', 12)
        it.dWidgets[j] = Object.assign(set, jt)
      })
    })
    store.state.dPagesList = data.pages
    store.state.dWidgets = store.state.dPagesList[0].dWidgets
  },
  /**
  * 选择页面
  */
  selectCurrentPage (store, index) {
    store.state.dWidgets = store.state.dPagesList[index].dWidgets
    store.state.dPagesIndex = index
    store.state.dHistory = []
    store.state.dActiveUuidHistory = []
  ...
  * 删除页面
  */
  deletePage (store, index) {
    if (store.state.dPagesList.length > 1) {
      store.state.dPagesList.splice(index, 1)
      if (store.state.dPagesIndex === index) {
        store.state.dWidgets = store.state.dPagesList[0].dWidgets || []
        store.state.dHistory = []
        store.state.dActiveUuidHistory = []
        store.state.dSelectWidgets = []
  ...
  */
  addPage (store) {
    store.state.dPagesList.push({
      uuid: generate('1234567890abcdef', 12),
      dWidgets: []
    })
  },
  /**
  ...
  handleHistory (store, action) {
    ...
    let index = store.state.dPagesIndex
    store.state.dPagesList[index].dWidgets = store.state.dWidgets
  },
  ...
  initDResize (store, payload) {
    ...
    resizeWH.rotate = payload.rotate
  },
  // 更新组件宽高
  dResize (store, { x, y, dirs, index, obox }) {
    ...
    if (dirs[0] === 'rotate') {
      let cientRects = document.querySelector('#size-control>.center').getClientRects()[0]
      let ox = cientRects.x
      let oy = cientRects.y
      target.rotate = parseInt(-Math.atan2(x - ox, y - oy) / Math.PI * 180 + 180)
    } else {
      ...
      dx *= 100 / store.state.dZoom
      dy *= 100 / store.state.dZoom
      for (let i = 0; i < dirs.length; ++i) {
        let dir = dirs[i]
        switch (dir) {
          case 'top':
            target.height = Math.max(-dy + resizeWH.height, 0)
            break
          case 'bottom':
            target.height = Math.max(dy + resizeWH.height, 0)
            break
          case 'left':
            target.width = Math.max(-dx + resizeWH.width, 0)
            break
          case 'right':
            target.width = Math.max(dx + resizeWH.width, 0)
            break
      ...
      let box = document.querySelectorAll('#size-control .square')[index].getClientRects()[0]
      let mx = Math.round(obox.x - box.x)
      let my = Math.round(obox.y - box.y)
      target.left += mx
      target.top += my
    }
